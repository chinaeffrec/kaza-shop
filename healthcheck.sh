...
# ── Запись восстановления (сбросить состояние «упало») ───────────────────────
mark_recovered() {
    local key="$1"
...
if [[ "$API_STATUS" == "200" ]]; then
    ...
    if mark_recovered "api"; then
        send_alert "✅ <b>Магазин восстановлен</b>%0A%0AAPI снова отвечает нормально."
    fi
else
    ...
    if should_alert "api_down"; then
        send_alert "🔴 <b>Магазин не отвечает!</b>%0A%0AAPI статус: ${API_STATUS}%0A%0AПопытка автоматического перезапуска..."
        # Попытка автовосстановления
        cd "$INSTALL_DIR"
        docker compose -f docker-compose.prod.yml restart app >/dev/null 2>&1 || true
        sleep 15
        RETRY=$(curl -sf -o /dev/null -w "%{http_code}" \
            --max-time 10 "http://localhost:8000/health" 2>/dev/null || echo "000")
        if [[ "$RETRY" == "200" ]]; then
            send_alert "✅ <b>Автовосстановление успешно</b>%0A%0AAPI работает после перезапуска."
            mark_recovered "api" >/dev/null 2>&1 || true
        else
            send_alert "🆘 <b>Автовосстановление не помогло</b>%0A%0AТребуется ручное вмешательство.%0A%0AСправка: bash ${INSTALL_DIR}/update.sh"
        fi
    fi
fi
...
cd "$INSTALL_DIR"
STOPPED=$(docker compose -f docker-compose.prod.yml ps --status exited \
...
if [[ "$STOPPED" -gt 0 ]]; then
    ...
    if should_alert "containers_down"; then
        NAMES=$(docker compose -f docker-compose.prod.yml ps --status exited \
            --format "{{.Name}}" 2>/dev/null | tr '\n' ', ' || echo "?")
        send_alert "⚠️ <b>Упавшие контейнеры: ${STOPPED}</b>%0A%0AКонтейнеры: ${NAMES}%0A%0AПерезапуск..."
...
if [[ "$DISK_USED" -ge 90 ]]; then
    ...
    if should_alert "disk_full"; then
        ...
        send_alert "💾 <b>Диск почти полон: ${DISK_USED}%</b>%0A%0AИспользовано: ${DISK_INFO}%0A%0AПочистите старые логи или бэкапы."
    fi
elif [[ "$DISK_USED" -ge 80 ]]; then
    echo "${LOG_PREFIX}: ПРЕДУПРЕЖДЕНИЕ — диск на ${DISK_USED}%"
    if should_alert "disk_warn"; then
        send_alert "💾 <b>Внимание: диск заполнен на ${DISK_USED}%</b>%0A%0AРекомендуется проверить и освободить место."
    fi
else
    ...
    if [[ -n "$EXPIRY" ]]; then
        ...
        if [[ "$DAYS_LEFT" -le 14 && "$DAYS_LEFT" -gt 0 ]]; then
            if should_alert "ssl_expiry"; then
                send_alert "🔒 <b>SSL-сертификат истекает через ${DAYS_LEFT} дней</b>%0A%0AОбновление должно происходить автоматически. Если нет — выполните:%0A<code>certbot renew</code>"
            fi
        fi
    fi
fi
...
if [[ "$RAM_FREE_MB" -lt 100 ]]; then
    ...
    if should_alert "ram_low"; then
        send_alert "🧠 <b>Мало свободной памяти: ${RAM_FREE_MB} MB</b>%0A%0AЕсли проблема повторяется — рассмотрите увеличение RAM на VDS."
    fi
else
