"""
Сервис загрузки файлов настроек (лого, штамп, QR).
"""
import uuid
...
async def read_settings(session: AsyncSession) -> SettingsResponse:
    s = await get_shop_settings(session)
    return _to_response(s)


async def update_settings(data: SettingsUpdate, session: AsyncSession) -> SettingsResponse:
    s = await get_shop_settings(session)
...
async def list_faq(session: AsyncSession) -> list[FaqItemResponse]:
...
async def create_faq(data, session: AsyncSession) -> FaqItemResponse:
...
async def update_faq(item_id: int, data: FaqUpdate, session: AsyncSession) -> FaqItemResponse:
...
async def delete_faq(item_id: int, session: AsyncSession) -> dict:
    res = await session.execute(select(FaqItem).where(FaqItem.id == item_id))
    item = res.scalar_one_or_none()
    if not item:
        raise HTTPException(404, "Not found")
