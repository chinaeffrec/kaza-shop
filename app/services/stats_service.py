...
async def get_dashboard(
    date_from: Optional[str], date_to: Optional[str], session: AsyncSession
) -> DashboardResponse:
...
async def export_orders_xlsx(
    ...
    status: Optional[str], session: AsyncSession
) -> BytesIO:
    q = select(Order).order_by(Order.created_at.desc())
    if date_from:
        q = q.where(Order.created_at >= datetime.fromisoformat(date_from))
...
async def get_product_stats(
    ...
    # Средняя цена = выручка / количество проданных за период
    # (только реальные продажи, без отменённых и без непроданных товаров)
...
async def export_product_stats_xlsx(
    ...
    result = await get_product_stats(date_from, date_to, session)
