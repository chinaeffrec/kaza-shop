...
async def _get_or_create_subcategory(session: AsyncSession, cat_id: int, name: str) -> SubCategory:
    name = name.strip()
    res = await session.execute(
        select(SubCategory).where(SubCategory.name == name, SubCategory.category_id == cat_id)
    )
...
async def import_products(content: bytes, session: AsyncSession) -> ImportResponse:
    ...
    try:
        for idx, row in df.iterrows():
            try:
                ...
                cat = await _get_or_create_category(session, str(row["category"]))
                sub = await _get_or_create_subcategory(session, cat.id, str(row["subcategory"]))
                ...
                res = await session.execute(
                    select(Product).where(Product.name == name, Product.subcategory_id == sub.id)
                )
                ...
                if existing:
                    ...
                    session.add(Product(subcategory_id=sub.id, name=name, **kwargs))
                    created += 1
