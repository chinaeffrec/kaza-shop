...
export default function ProductsPage() {
  ...
  function openAdd() { setForm(EMPTY); setPhotoFile(null); setPhotoPreview(null); setModal('add') }

  function openEdit(p) {
    const sub = allSubcats[p.subcategory_id]
    setForm({
      name:p.name, price:String(p.price), discount_price:String(p.discount_price||''),
      subcategory_id:String(p.subcategory_id), description:p.description||'',
      characteristics:p.characteristics||'', stock:p.stock||0,
      is_active:p.is_active, _cat_id: sub ? String(sub.category_id) : ''
    })
    setPhotoFile(null); setPhotoPreview(p.image_url ? api.BASE + p.image_url : null)
    setModal(p)
  }
  ...
  return (
    <div>
      ...
      <div className={s.filters}>
        ...
        <select className={s.filterSelect} value={filterCat} onChange={e=>setFilterCat(e.target.value)}>
          <option value="">Все категории</option>
          {categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        {filterSubOpts.length > 0 && (
          <select className={s.filterSelect} value={filterSub} onChange={e=>setFilterSub(e.target.value)}>
          ...
          <tbody>
            {visible.map(p => (
              <tr key={p.id} className={!p.has_image ? s.noPhotoRow : ''}>
                ...
                <td>
                  <button
                    className={p.is_active ? s.activeBadge : s.inactiveBadge}
          ...
          <div className={s.modalBox}>
            ...
            <div style={{display:'flex',gap:12}}>
              ...
              <label className={s.label} style={{flex:1}}>Цена со скидкой
                <input className={s.input} type="number" value={form.discount_price} onChange={e=>setForm(f=>({...f,discount_price:e.target.value}))} />
              </label>
            </div>
            ...
            <label className={s.label}>Остаток (шт)
              <input className={s.input} type="number" min="0" value={form.stock} onChange={e=>setForm(f=>({...f,stock:e.target.value}))} />
            </label>

            <label className={s.label}>Фото (JPEG/PNG/WebP)
              {photoPreview && (
                <img src={photoPreview} alt="" style={{width:80,height:80,objectFit:'cover',borderRadius:6,marginBottom:6}} />
              )}
